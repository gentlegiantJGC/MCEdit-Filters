...
#Modified and updated by gentlegiantJGC
...
#If you do , can you quote us : xafonyz, https://www.youtube.com/xafonyzen and gentlegiantJGC, https://www.youtube.com/gentlegiantJGC
...
displayName = "one-command structure spawner 2.0 (Sparks)" 
...
("Absolute Y in relative mode", True),
("Main", "title"),),
	...
	for (chunk, slices, point) in level.getChunkSlices(box):
		if options["Include blocks"] == True:
			for t in chunk.TileEntities:
				...
				z = t["z"].value
				if (x,y,z) in box:
					if options["LastOutput"] == True and "LastOutput" in t:
							del t["LastOutput"]
							
					if options["SuccessCount"] == True and "SuccessCount" in t:
							del t["SuccessCount"]
							
					if options["TrackOutput"] == True and "TrackOutput" in t:
							del t["TrackOutput"]
		if options["Include entities"] == True:
			for e in chunk.Entities:
				...
				if (x,y,z) in box:
					...
					if options["SuccessCount"] == True and "SuccessCount" in e:
						del t["SuccessCount"]
						
					if options["TrackOutput"] == True and "TrackOutput" in e:
						del t["TrackOutput"]
					...
					if options["AbsorptionAmount"] == True and "AbsorptionAmount" in e:
						del e["AbsorptionAmount"]
						
					if options["Air"] == True and "Air" in e:
						del e["Air"]
						
					if options["Attributes"] == True and "Attributes" in e:
						del e["Attributes"]
						
					if options["DeathTime"] == True and "DeathTime" in e:
						del e["DeathTime"]
						
					if options["Dimension"] == True and "Dimension" in e:
						del e["Dimension"]
						
					if options["Fire"] == True and "Fire" in e:
						del e["Fire"]
						
					if options["HurtByTimestamp"] == True and "HurtByTimestamp" in e:
						del e["HurtByTimestamp"]
						
					if options["PortalCooldown"] == True and "PortalCooldown" in e:
						del e["PortalCooldown"]
						
					if options["CustomName"] == True and "CustomName" in e:
						del e["CustomName"]
						
					if options["Small"] == True and "Small" in e:
						del e["Small"]
					
					if options["ShowArms"] == True and "ShowArms" in e:
						del e["ShowArms"]
						
					if options["Motion"] == True and "Motion" in e:
						del e["Motion"]
						
					if options["Rotation"] == True and "Rotation" in e:
						del e["Rotation"]
						
					if options["HurtTime"] == True and "HurtTime" in e:
						del e["HurtTime"]
						
					if options["NoBasePlate"] == True and "NoBasePlate" in e:
						del e["NoBasePlate"]
						
					if options["HealF"] == True and "HealF" in e:
						del e["HealF"]
						
					if options["Health"] == True and "Health" in e:
						del e["Health"]
						
					if options["OnGround"] == True and "OnGround" in e:
						del e["OnGround"]
						
					if options["FallDistance"] == True and "FallDistance" in e:
						del e["FallDistance"]
						
					if options["Marker"] == True and "Marker" in e:
						del e["Marker"]		
			...
			for x in xrange(box.minx,box.maxx):
				...
				for z in xrange(box.minz,box.maxz):
					...
					if cmdBlockPosing=="Absolute":
						...
						if options["Absolute Y in relative mode"] == True:
							blockY = y
						else:
					...
					if includeBlocks==True and level.blockAt(x,y,z)!=0:
						if level.tileEntityAt(x,y,z)==None:
							cmdtemp='{id:MinecartCommandBlock,Command:"setblock'+ {True: " ~", False: " "}[cmdBlockPosing=="Relative"] +str(blockX)+{True: " ", False:{True: " ~", False: " "}[cmdBlockPosing=="Relative"]}[options["Absolute Y in relative mode"] == True]+str(blockY)+{True: " ~", False: " "}[cmdBlockPosing=="Relative"]+str(blockZ)+' '+IDsToName[str(level.blockAt(x,y,z))]+' '+str(level.blockDataAt(x,y,z))+{True: " destroy", False: " replace"}[level.blockAt(x,y,z)==152]+'",Riding:'
					...
					if includeAir==True and level.blockAt(x,y,z)==0:
						if level.tileEntityAt(x,y,z)==None:
							cmdtemp='{id:MinecartCommandBlock,Command:"setblock'+ {True: " ~", False: " "}[cmdBlockPosing=="Relative"]+str(blockX)+{True: " ", False:{True: " ~", False: " "}[cmdBlockPosing=="Relative"]}[options["Absolute Y in relative mode"] == True]+str(blockY)+{True: " ~", False: " "}[cmdBlockPosing=="Relative"]+str(blockZ)+' '+IDsToName[str(level.blockAt(x,y,z))]+' '+str(level.blockDataAt(x,y,z))+' replace",Riding:'

						if level.tileEntityAt(x,y,z) != None:
							cmdtemp='{id:MinecartCommandBlock,Command:"setblock'+ {True: " ~", False: " "}[cmdBlockPosing=="Relative"]+str(blockX)+{True: " ", False:{True: " ~", False: " "}[cmdBlockPosing=="Relative"]}[options["Absolute Y in relative mode"] == True]+str(blockY)+{True: " ~", False: " "}[cmdBlockPosing=="Relative"]+str(blockZ)+' '+IDsToName[str(level.blockAt(x,y,z))]+' '+str(level.blockDataAt(x,y,z))+' replace {'+NBT2Command(level.tileEntityAt(x,y,z))+'}",Riding:'
